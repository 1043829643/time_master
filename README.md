...
- 切回页面或其他标签页保存时自动同步；未发送文字保留在当前浏览器标签页，刷新后可继续。识别失败的录音保留在当前面板内，可点击重试识别；刷新或关闭面板会释放录音。
...
npm run build
node --experimental-strip-types --test tests/domain.test.mjs tests/reliability.test.mjs
