// Synchronous SHA-256 keeps browser and Worker concurrency tokens identical.
// Input is UTF-8; this is an integrity fingerprint, never an authorization token.
...
export function fingerprint(text:string):string {
 const input=new TextEncoder().encode(text),bytes=new Uint8Array(Math.ceil((input.length+9)/64)*64);bytes.set(input);bytes[input.length]=128;
 const view=new DataView(bytes.buffer);view.setUint32(bytes.length-8,Math.floor(input.length/0x20000000));view.setUint32(bytes.length-4,(input.length*8)>>>0);
