...
export async function qwen(body:unknown,kind:'chat'|'tts'|'asr'='chat'){
 ...
 const url=host+path;
 let response:Response;try{response=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${env.QWEN_API_KEY}`,'Content-Type':'application/json',...(config.tokenPlan&&kind==='asr'?{'X-DashScope-SSE':'disable'}:{})},body:JSON.stringify(body),signal:AbortSignal.timeout(60000)})}catch{throw new ApiError('Qwen 连接超时或网络不可用，请重试。',502)}
