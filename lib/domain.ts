...
export type Snapshot={data:Data;revision:number;scope?:string};
...
export function applyOperations(current:Data,raw:unknown,operationId:string,summary='更新工作空间'):Data{
 ...
 const removeTask=(taskId:string)=>{d.tasks=d.tasks.filter(t=>t.id!==taskId).map(t=>({...t,dependencies:t.dependencies.filter(x=>x!==taskId)}));d.blocks=d.blocks.filter(b=>b.taskId!==taskId||b.done||b.fixed).map(b=>b.taskId===taskId?{...b,taskId:''}:b);};
