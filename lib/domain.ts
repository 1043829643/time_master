...
export function applyOperations(current:Data,raw:unknown,operationId:string,summary='更新工作空间'):Data{
 const ops=structuredClone(z.array(operationSchema).min(1).max(60).parse(raw));const d=structuredClone(current);if(d.appliedIds.includes(operationId))return d;
