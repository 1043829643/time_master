...
export type SaveOptions={baseline?:ChangeBaseline;workRevision?:number;operationId?:string;onConflict?:(detail:ConflictDetails)=>void;onClosed?:()=>void;onCommitted?:(snapshot:Snapshot,receipt?:OperationReceipt)=>void};
...
export function useWorkspace(){
 ...
 const receive=useCallback((incoming:Snapshot)=>{const s={...incoming,scope:incoming.scope||latest.current.scope};if(s.revision>=latest.current.revision){const newer=s.revision>latest.current.revision;latest.current=s;setSnapshot(s);if(newer)channel.current?.postMessage({revision:s.revision})}setLoaded(true);setSyncState('synced');setLastSynced(new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'}))},[]);
 const sync=useCallback(async()=>{try{const r=await requestJson<Snapshot&{unchanged?:boolean}>('/api/workspace'+(latest.current.revision>0?'?since='+latest.current.revision:''));const s=r.unchanged?latest.current:r;receive(s);return s}catch(e){setSyncState(navigator.onLine?'error':'offline');throw e}},[receive]);
 ...
 const save:SaveWorkspace=useCallback(async(operations,summary,options={})=>{
  ...
  const body=saveAttempt.current.body;
   for(let attempt=0;attempt<4;attempt++){
    try{const s=await requestJson<Snapshot&{receipt?:OperationReceipt}>('/api/workspace',body);receive(s);saveAttempt.current=null;options.onCommitted?.(s,s.receipt);notify('已保存：'+summary);return true}
    catch(e){
     ...
     if(e instanceof ClientError&&e.status===409)saveAttempt.current=null;if(e instanceof ClientError&&e.details?.code==='OPERATION_CLOSED')options.onClosed?.();
