...
export async function POST(req:Request){try{guardOrigin(req);const user=await owner(req),body=z.object({revision:z.number().int().min(0),operations:z.array(z.unknown()).min(1).max(60),operationId:z.string().min(8).max(100),summary:z.string().max(300).default('更新工作空间')}).parse(await readJson(req));return Response.json(await writeWorkspace(user,body.revision,body.operations,body.operationId,body.summary));}catch(e){return errorResponse(e)}}
